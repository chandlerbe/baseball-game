namespace BaseballGame {
  export class BattingStats extends Stats {
    plateAppearances: KnockoutObservable<number> = ko.observable(0).extend({ min: 0, max: 99 });
    runsScored: KnockoutObservable<number> = ko.observable(0).extend({ min: 0, max: 99 });
    runsBattedIn: KnockoutObservable<number> = ko.observable(0).extend({ min: 0, max: 99 });
