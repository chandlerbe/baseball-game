namespace BaseballGame {
  export class Scoreboard {
    numberOfOuts: KnockoutObservable<number> = ko.observable(0).extend({ min: 0, max: 3 });
    balls: KnockoutObservable<number> = ko.observable(0).extend({ min: 0, max: 4 });
    strikes: KnockoutObservable<number> = ko.observable(0).extend({ min: 0, max: 3 });
    visitorTeamRuns: KnockoutObservableArray<number> = ko.observableArray([0]);
    visitorTeamHits: KnockoutObservable<number> = ko.observable(0).extend({ min: 0, max: 99 });
    visitorTeamErrors: KnockoutObservable<number> = ko.observable(0).extend({ min: 0, max: 99 });
    homeTeamRuns: KnockoutObservableArray<number> = ko.observableArray([]);
    homeTeamHits: KnockoutObservable<number> = ko.observable(0).extend({ min: 0, max: 99 });
    homeTeamErrors: KnockoutObservable<number> = ko.observable(0).extend({ min: 0, max: 99 });
